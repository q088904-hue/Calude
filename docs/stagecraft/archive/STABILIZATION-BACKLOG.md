...
|---|---|---|---|---|---|
| Mobile | P2 | **done** | Hub overflowed horizontally at narrow mobile (scrollWidth ~640 at 375) | `NavDropdown.tsx`, `page.tsx` header | FIXED: header + NavDropdown nav now `flex-wrap` (+ `gap-y-2`), so on narrow widths the nav wraps instead of forcing page overflow. No clip, all content visible; desktop/tablet have room → visually unchanged. **Verified:** hub at 375 → scrollWidth 375, no offenders. |
| Mobile | P3 | **done** | Same overflow on interior pages | `StagecraftHeader.tsx` | FIXED by the same `flex-wrap` change applied to `StagecraftHeader`. **Verified:** `/stagecraft/quickfire` at 375 → no overflow. |
...
| Reliability | P2 | **done** | Streak counter computed wrong/zero (`computeStreak`) | `src/app/stagecraft/page.tsx` | OBS-10. FIXED: chronological sort (by midnight timestamp, de-duped) + `today` normalized to local midnight so time-of-day no longer skews the diff. Was a lexicographic `toDateString()` sort ("Thu"<"Tue"<"Wed") **plus** a time-of-day rounding bug. **Verified (deterministic):** 3 consecutive incl today → 3 (was 0); reversed order → 3; gap → 1; no-today → 0; dupes → 2. |
