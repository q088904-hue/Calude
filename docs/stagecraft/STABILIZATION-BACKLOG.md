...
| Reliability | P3→P4 | obs (root-caused) | `setState` synchronously in effect — ESLint `react-hooks` | `DailyPracticeTracker`, `src/app/stagecraft/page.tsx:1110` | **ROOT CAUSE (investigated, no fix applied):** intentional post-hydration `localStorage` read. Component renders null until a mount effect reads streak + today's session/quickfire counts and sets state (gated by `mounted`) — the standard pattern to avoid an SSR hydration mismatch (server `0` vs client value). The 4 setStates batch into ONE extra re-render; impact negligible. Lint is conservative. **Proposed fixes (pick one, needs approval): (A)** `eslint-disable-next-line` + comment documenting the intentional pattern — zero behavior change; **(B)** refactor to `useSyncExternalStore` (matches ECC react/hooks guidance, removes the extra render + the warning) — slightly more code; **(C)** lazy `useState(() => read())` — **REJECTED**, reintroduces the hydration mismatch the `mounted` flag prevents. Recommend **B** (idiomatic) or **A** (minimal). |
